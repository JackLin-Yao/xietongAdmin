...
const openEdit = (row, column, cell, event) => {
  if (
    ...
    JSON.stringify(multipleSelection.value).indexOf(JSON.stringify(row)) > 0
  ) {
    isEdit.value = true;
  } else {
    isEdit.value = false;
  ...
  <div class="poNumDetaile">
    <el-card class="box-card">
      ...
      <div class="detail-list">
        ...
        <div class="detail-list-item">
          供应商：<el-tag size="large">{{ companyName }}</el-tag>
        </div>
      </div>
    </el-card>
    <el-card class="boxList-card">
      ...
      <el-table
        ...
        @select-all="selectCheckBoxAllAtion"
        @select="selectCheckBoxAtion"
        @selection-change="selectionAtion"
      >
        <el-table-column type="selection" width="55"></el-table-column>
        <el-table-column label="项次" prop="poLine" align="left" width="55">
        </el-table-column>
        ...
        <el-table-column label="请购单行备注" prop="utNote" align="left">
        </el-table-column>
        ...
        <el-table-column label="发货数量" prop="qtyOrdered" align="center" width="200">
          <template #default="scope">
            ...
            <el-input-number
              v-if="
                isEdit &&
                JSON.stringify(multipleSelection).includes(JSON.stringify(scope.row))
              "
              v-model="scope.row.qtyDeliv"
              :min="0"
              :max="scope.row.qtyUndeliv"
            />
        ...
        <el-card style="width: 1250px">
          <div id="printMe" ref="printRef" style="width: 1200px">
            <div class="dialog-header" style="width: 1200px">
              <div>
                <span class="companyNmae">{{ companyName }}</span
                ><br />
            ...
            <div class="table-header" style="width: 1200px">
              ...
              <div class="table-header-right"><!-- <div></div> -->1111111111111</div>
            </div>

            <TableView
              :data="multipleSelection"
              :options="DNoption"
              style="width: 1170px; page-break-after: always"
            >
            </TableView>
            <!-- <table>
              <thead>
                <tr>
                  <template v-for="labelitem in DNoption" :key="labelitem.label">
              ...
              <tbody>
                <tr>
                  <template v-for="(tableitem, index) in multipleSelection" :key="index">
                    <td>{{ index }}</td>
                  </template>
                </tr>
                <tr>
                  <template v-for="(tableitem, index) in multipleSelection" :key="index">
                    <td>{{ tableitem.poNum }}</td>
                  </template>
                </tr>
                <tr>
                  <template v-for="(tableitem, index) in multipleSelection" :key="index">
                    <td>{{ tableitem.poLine }}</td>
                  </template>
                </tr>
                <tr>
                  <template v-for="(tableitem, index) in multipleSelection" :key="index">
                ...
                <tr>
                  <template v-for="(tableitem, index) in multipleSelection" :key="index">
                    <td>{{ tableitem.itemDesc }}</td>
                  </template>
                </tr>
                <tr>
                  <template v-for="(tableitem, index) in multipleSelection" :key="index">
                    <td>{{ tableitem.qtyDeliv }}</td>
                  </template>
                </tr>
              </tbody>
            </table> -->
